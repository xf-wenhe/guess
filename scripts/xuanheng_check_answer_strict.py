...
def check_answer(
    ...
    if len(hints7) == 7:
        ...
        if any(length < 2 or length > MAX_HINT_CHARS for length in lengths):
            issues.append({
                "type": "hint_length",
                "hints": hints7,
                "lengths": lengths,
        ...
        seen_hints: dict[str, int] = {}
        for slot_idx, hint in enumerate(hints7, start=1):
            if hint in seen_hints:
                other_slot = seen_hints[hint]
                issues.append({
                    "type": "same_item_duplicate",
                    "slot": slot_idx,
                    "other_slot": other_slot,
                    "hint": hint,
                })
            else:
                seen_hints[hint] = slot_idx
...
def main() -> int:
    ...
    puzzles = json.loads(puzzles_path.read_bytes().decode("utf-8-sig"))
