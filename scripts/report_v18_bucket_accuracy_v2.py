...
CALIB_OLD = Path('data/semantic_calibration_v18.json')
...
def summarize(name, pred, truth):
    ...
    for key in labels:
        n = per_total[key]
        if n == 0:
            print(f'  {key}: total=0 hit=0')
            continue
        print(f'  {key}: {per_hit[key]}/{n} ({per_hit[key]/n*100:.1f}%)')
...
def main():
    ...
    old = json.loads(CALIB_OLD.read_text(encoding='utf-8'))
    old_pred = np.array([apply_calibration(float(p), old['x_pred'], old['y_calibrated']) for p in raw], dtype=np.float32)

    v2 = json.loads(CALIB_V2.read_text(encoding='utf-8'))
    v2_pred = np.array([apply_calibration(float(p), v2['x_pred'], v2['y_calibrated']) for p in raw], dtype=np.float32)

    print(f'rows={len(rows)}')
    summarize('raw', raw, truth)
    summarize('calib_v1', old_pred, truth)
    summarize('calib_v2', v2_pred, truth)
