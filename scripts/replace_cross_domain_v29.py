...
SAFE_BY_CATEGORY = {
    ...
    "动漫": ["剧情线索", "角色线索", "设定元素", "关键桥段", "冲突关系", "叙事节点", "作品辨识"],
    ...
    "神话": ["传说线索", "神祇背景", "故事母题", "象征意涵", "神祇冲突", "文化源流", "叙事结构"],
    "动作": ["动作线索", "执行方式", "发力路径", "姿态变化", "步骤衔接", "结果反馈", "场景触发"],
...
def run_replace(input_path: Path, out_dir: Path, apply: bool) -> dict:
    ...
    for item in data:
        ...
        for i, h in enumerate(hints[:7]):
            cross = detect_cross_domain(category, h)
            if cross:
                safe = pick_safe(category, used, i)
                used.add(safe)
                new_hints.append(safe)
                replaced_hints += 1
                item_changed = True
            else:
                new_hints.append(h)
        ...
        if len(new_hints) < 7:
            for i in range(7 - len(new_hints)):
                safe = pick_safe(category, set(new_hints), i)
                new_hints.append(safe)

        if new_hints != hints[:7]:
            item["hints"] = new_hints[:7]
            changed_items += 1
            if not item_changed:
                item_changed = True

    out_dir.mkdir(parents=True, exist_ok=True)
    snapshot = out_dir / "puzzles.cross_domain_replaced.v29.json"
    report = out_dir / "hints_cross_domain_replaced_v29_report.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Replace cross-domain hints v29")
