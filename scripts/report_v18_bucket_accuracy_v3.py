...
CALIB_FILES = {
    ...
    'calib_v3': Path('data/semantic_calibration_v18_v3.json'),
}
...
def main():
    ...
    for name, path in CALIB_FILES.items():
        c = json.loads(path.read_text(encoding='utf-8'))
        pred = np.array([apply_calibration(float(p), c['x_pred'], c['y_calibrated']) for p in raw], dtype=np.float32)
        summarize(name, pred, truth)
