...
CALIB_CSV = Path(os.getenv('SEM_CALIB_CSV', 'data/splits/semantic_calib.csv'))
MODEL_PATH = os.getenv('SEM_MODEL_PATH', 'models/bge-m3-finetuned-v19')
OUTPUT_JSON = Path(os.getenv('SEM_CALIB_OUTPUT_JSON', 'data/semantic_calibration_v19.json'))
...
def compress_points(x_sorted, y_iso, max_points=120):
    ...
    if xs and xs[-1] < float(x_sorted[-1]):
...
def main():
    rows = load_rows(CALIB_CSV)
    if len(rows) < 50:
        raise SystemExit('not enough calibration rows, run split_semantic_dataset.py first')
    ...
    payload = {
        'model': MODEL_PATH,
        'method': 'weighted_isotonic_split_v19',
        'x_pred': x_points,
