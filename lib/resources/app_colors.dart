import 'package:flutter/material.dart';

class AppColors {
  const AppColors._();

  static const transparent = Colors.transparent;
  static const white = Colors.white;
  static const black12 = Colors.black12;
  static const black54 = Colors.black54;

  static const seed = Color(0xFF0EA5A6);
  static const scaffoldBackground = Color(0xFFF6F9F7);
  static const appBarForeground = Color(0xFF0F2A2A);
  static const cardBackground = Color(0xFFFCFFFE);
  static const cardBorder = Color(0xFFD7EEE6);
  static const snackBarBackground = Color(0xFF093533);

  static const primaryTeal = Color(0xFF14B8A6);
  static const primaryTealDark = Color(0xFF0F766E);
  static const primaryTealSoft = Color(0xFFE8F6F2);
  static const primaryTealBorder = Color(0xFFB6E7DA);
  static const primaryTealLight = Color(0xFFE0F7FA);
  static const primaryTealShadow = Color(0x3300B894);
  static const primaryTealShadowSoft = Color(0x2200B894);
  static const primaryMint = Color(0xFF5EEAD4);
  static const primaryMintSoft = Color(0xFFCCFBF1);
  static const primaryMintBorder = Color(0xFFD1FAE5);

  static const primaryBlue = Color(0xFF2563EB);
  static const primaryBlueBright = Color(0xFF3B82F6);
  static const primaryBlueDark = Color(0xFF145DFF);
  static const primaryBlueDeep = Color(0xFF1D4ED8);
  static const primaryBlueText = Color(0xFF1E3A8A);
  static const primaryBlueSoft = Color(0xFFF1F5FF);
  static const primaryBlueBorder = Color(0xFFD3DDF9);
  static const primaryBlueNotice = Color(0xFFEFF4FF);
  static const primaryBlueNoticeBorder = Color(0xFFB7D1FF);
  static const primaryBlueNoticeText = Color(0xFF1B4FCC);
  static const primaryBlueShadow = Color(0x332563EB);
  static const primaryBlueTextSoft = Color(0xFF5C77C7);
  static const primaryBlueBadge = Color(0xFFE8F1FF);
  static const primaryBlueBadgeBorder = Color(0xFFE3E8EF);

  static const success = Color(0xFF16A34A);
  static const successText = Color(0xFF15803D);
  static const successDeepText = Color(0xFF166534);
  static const successBg = Color(0xFFF7FFF9);
  static const successBorder = Color(0xFFBBF7D0);
  static const successShadow = Color(0x3316A34A);
  static const successChipBg = Color(0xFFECFDF3);
  static const successChipBorder = Color(0xFFDCFCE7);
  static const successBgSoft = Color(0xFFF0FDF4);

  static const danger = Color(0xFFDC2626);
  static const dangerText = Color(0xFFB42318);
  static const dangerBg = Color(0xFFFFFAFA);
  static const dangerBorder = Color(0xFFF6D6D6);
  static const dangerShadow = Color(0x33291E1E);
  static const dangerChipBg = Color(0xFFFFEFEF);
  static const dangerChipBorder = Color(0xFFFECACA);
  static const dangerBgSoft = Color(0xFFFFF2F2);
  static const dangerNoticeBg = Color(0xFFFFF4F4);
  static const dangerNoticeBorder = Color(0xFFFFC1C1);

  static const warningBg = Color(0xFFFFF7E6);
  static const warningBorder = Color(0xFFFFD38A);
  static const warningText = Color(0xFFB54708);

  static const textPrimary = Color(0xFF0F172A);
  static const textSecondary = Color(0xFF64748B);
  static const textMuted = Color(0xFF94A3B8);
  static const textStrong = Color(0xFF111827);
  static const textStatusLight = Color(0xFFDDE9FF);
  static const textDisabled = Color(0xFFCBD5E1);

  static const inputBackground = Color(0xFFF8FCFB);
  static const inputBorder = Color(0xFFD3E9E2);
  static const inputFill = Color(0xFFF3F6FB);
  static const inputOutline = Color(0xFFD5DEEE);
  static const sheetInputFill = Color(0xFFF2F4F7);

  static const historyRow = Color(0xFFFCFEFF);
  static const historyRowEmpty = Color(0xFFF7FAFF);
  static const historyRowBorder = Color(0xFFD9E6F5);

  static const overlayBarrier = Color(0x40121A2B);
  static const panelSurface = Color(0xFFF6FAFB);
  static const neutralSurface = Color(0xFFF8FAFF);
  static const neutralBorder = Color(0xFFC9DAFF);
  static const neutralLine = Color(0xFFE2E8F0);
  static const neutralGradientStart = Color(0xFFF1F5F9);

  static const decorMint = Color(0xFFE8F7F2);
  static const decorIvory = Color(0xFFF5FBF9);
  static const decorWarm = Color(0xFFF9F7EF);
  static const decorAmber = Color(0xFFF59E0B);

  static const explosionPalette = <Color>[
    Color(0xFFFF3B30),
    Color(0xFFFF8A00),
    Color(0xFFFFD166),
    Color(0xFFFF6B6B),
    Color(0xFFFFC857),
  ];

  static const starBurstPalette = <Color>[
    Color(0xFFFFD54F),
    Color(0xFFFFF59D),
    Color(0xFFFFC107),
    Color(0xFFFFE082),
  ];

  // Score heatmap colors
  static const scoreGray = Color(0xFF94A3B8);   // 0–29%  无关联
  static const scoreGreen = Color(0xFF16A34A);  // 60–79% 中关联
  static const scoreAmber = Color(0xFFF59E0B);  // 80–94% 高关联
  static const scoreGold = Color(0xFFD97706);   // 95–100% 极近

  static const scoreGrayBg = Color(0xFFF1F5F9);
  static const scoreGreenBg = Color(0xFFF0FDF4);
  static const scoreAmberBg = Color(0xFFFFFBEB);
  static const scoreGoldBg = Color(0xFFFEF3C7);
}